# TalentTrack: AI‑Powered Applicant Tracking System 🚀
...
*   **Storage:** Local JSON storage for resumes/generated resumes in dev (`backend/data/`), plus Firebase Admin for token verification.
...
## User Workflow 📊
A typical user interaction with TalentTrack follows these steps:
...
*   `/resumes`:
    *   `GET /`: Get list of uploaded resumes (supports authenticated + anonymous).
...
*   Firebase Project: used for Authentication (client) and token verification (admin).
...
# Option 1 (recommended): Path to service account JSON file
# FIREBASE_SERVICE_ACCOUNT_PATH="/absolute/path/to/serviceAccount.json"
# Option 2: Base64 encoded service account JSON string
# FIREBASE_SERVICE_ACCOUNT_BASE64="base64_of_serviceAccount_json"
#
# Local dev shortcut (gitignored):
# Place the JSON file anywhere under:
#   backend/keys/*.json
# The backend will auto-detect it.

# CORS Origin (Frontend URL)
CORS_ORIGIN="http://localhost:8080" # Or your deployed frontend URL
```
